...
function addMessage(opts = {}) {
  ...
  let { username = "", lastMsgGroup = null } = opts;
  let prom = Promise.resolve();
  if (lastMsgGroup === null) {
    ...
    if (lastMsgGroup === null || lastMsgGroup.name !== username) {
      ...
      prom = delay(50).then(() => showElement(lastMsgGroup.group));
    }
  }
...
function setChannel(name) {
  ...
  demoEle.channelsList
    .find((n) => n.innerText === name)
    .classList.add("selected");
  return delay(50);
}

function loadChannel(opts = {}) {
  let { name = "general", messages = [] } = opts;
...
window.addEventListener("load", () => {
  ...
  Object.values(users).forEach(
    (n) => (document.createElement("img").src = n.avatar)
  );
  ...
  delay(600)
    .then(() => loadChannel(selectedChannel))
    .catch(errorHandle);
});
  ...
  let result = text.replace(bracketsReplaceRegex, (match, tag, attributes) => {
    ...
    if (typeof attributes !== "number") {
      attr = attributes
        ...
        .filter((n) => n && n.trim())
        .reduce(
          (p, n) => (n[0] === '"' ? (p[p.length - 1] += n) : p.push(n), p),
          []
        )
        .map((n) => {
          let spl = n.split(/=/);
...
function createElement(opts = {}) {
  ...
  if (text) {
    ...
    let { result = text, changedBrackets = false } = hasBrackets
      ? parseBrackets(text)
      : {};
